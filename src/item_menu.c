...
bool8 UseRegisteredKeyItemOnField(u8 button)
{
    ...
    if(button == 0)
    {
        if (count != 0)
        {
            if(count == 1)
            {
                if (CheckBagHasItem(gSaveBlock1Ptr->registeredItems[0].itemId, 1) == TRUE)
                {
                    LockPlayerFieldControls();
                    FreezeObjectEvents();
                    PlayerFreeze();
                    StopPlayerAvatar();
                    gSpecialVar_ItemId = gSaveBlock1Ptr->registeredItems[0].itemId;
                    taskId = CreateTask(ItemId_GetFieldFunc(gSaveBlock1Ptr->registeredItems[0].itemId), 8);
    ...
    else
    {
        ...
        {
            if (CheckBagHasItem(gSaveBlock1Ptr->registeredItems[button-2].itemId, 1) == TRUE)
            {
                LockPlayerFieldControls();
                FreezeObjectEvents();
                PlayerFreeze();
                StopPlayerAvatar();
                gSpecialVar_ItemId = gSaveBlock1Ptr->registeredItems[button-2].itemId;
                taskId = CreateTask(ItemId_GetFieldFunc(gSaveBlock1Ptr->registeredItems[button-2].itemId), 8);
                gTasks[taskId].tUsingRegisteredKeyItem = TRUE;
                return TRUE;
            }
            else
